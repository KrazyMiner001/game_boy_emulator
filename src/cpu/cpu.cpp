...
namespace cpu {
    ...
    uint8_t CPU::perform_rot_operation(ROT_Instruction instruction, uint8_t value) {
        switch (instruction) {
            case ROT_Instruction::RLC: {
                uint8_t new_value = (value << 1) | (value >> 7);
                bool zero = new_value == 0;
                bool carry = value >> 7;
                registers.set_f(
                    (zero << 7) |
            ...
            break;
            case ROT_Instruction::RL: {
                bool old_carry = registers.get_f() & (1 << 4);

                bool carry = value >> 7;
                uint8_t new_value = (value << 1) | old_carry;
                bool zero = new_value == 0;
            ...
            break;
            case ROT_Instruction::RR: {
                bool old_carry = registers.get_f() & (1 << 4);

                bool carry = value & 1;
                uint8_t new_value = (value >> 1) | (old_carry << 7);
                bool zero = new_value == 0;
            ...
            break;
            case ROT_Instruction::SRA: {
                uint8_t new_value = (value >> 1) | (value & (1 << 7));
                bool carry = value & 1;
                bool zero = new_value == 0;
                registers.set_f(
                    (zero << 7) |
        ...
        if (is_prefix) {
            switch (helper.x) {
                case 0: {
                    ...
                    if (helper.z == 6) {
                        source_value = memory_bus.read(registers.get_hl());
                    } else {
                        source_value = registers.get_r8(Registers::from_r(helper.z));
                    }

                    uint8_t new_value = perform_rot_operation((ROT_Instruction) helper.y, source_value);

                    if (helper.z == 6) {
                        memory_bus.write(registers.get_hl(), new_value);
                    } else {
                        registers.set_r8(Registers::from_r(helper.z), new_value);
                    }
                }
                break;
                case 1: {
                ...
                case 2: {
                    ...
                    uint8_t new_value = source_value & (~(1 << helper.y));
